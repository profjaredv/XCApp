...
router.put('/:groupId', authenticate, requireTeam, async (req, res) => {
...
router.delete('/:groupId', authenticate, requireTeam, async (req, res) => {
  try {
    const existing = await prisma.meetGroup.findFirst({
      where: { id: req.params.groupId, teamId: req.user.teamId },
    });
...
router.post('/:groupId/races', authenticate, requireTeam, async (req, res) => {
...
router.delete('/:groupId/races/:raceId', authenticate, requireTeam, async (req, res) => {
  try {
    const group = await prisma.meetGroup.findFirst({ where: { id: req.params.groupId, teamId: req.user.teamId } });
    if (!group) {
      return res.status(404).json({ success: false, message: 'Meet group not found' });
